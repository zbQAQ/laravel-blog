...
class GoodsCate extends Model
{
    ...
    public function pushCate($goods, $cate, $field_name, $goods_cate, $field_id) {
        //    $goods是传进来的商品列  $cate是商品分类列  $field_name是商品列要新增字段  
        //    $goods_cate是商品的类别ID, $field_id是商品分类的id
        ...
        foreach($goods as $k => $v) {
            ...
            foreach($cate as $m => $n) {
                ...
                //并把商品类别对应的名字添加到 商品列里
                if( $v->$goods_cate === $n->$field_id ){
                    $goods[$k][$field_name] = $cate[$m]['gcate_name'];
                }
